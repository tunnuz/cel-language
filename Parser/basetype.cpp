...
BaseType::BaseType(const int& i) {
  m_type = BaseType::INT_TYPE;
  intValue = i;
}
BaseType::BaseType(const double& i) {
  m_type = BaseType::DOUBLE_TYPE;
  doubleValue = i;
}


BaseType::BaseTypeType BaseType::GiveType() {
  return (m_type);
}

double BaseType::evaluate() {
   switch (m_type) {
     case BaseType::INT_TYPE: return intValue; 
     case BaseType::DOUBLE_TYPE: return doubleValue; 
   }
